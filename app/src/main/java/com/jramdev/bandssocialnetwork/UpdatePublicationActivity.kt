...
class UpdatePublicationActivity : AppCompatActivity() {
    ...
    private var postDatabase: DatabaseReference? = null
...
//init Permissions arrays
        ...
        val query1: Query = postDatabase!!.orderByChild("pId")
            .equalTo(idPost)
        ...
        query1.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                for (ds in dataSnapshot.children) {
                    val description = "" + ds.child("pDescription").value
                    val gender = "" + ds.child("pGender").value
                    val title = "" + ds.child("pTitle").value

                    val spinnerPosition = adapter2.getPosition(gender)
                    listgender.setSelection(spinnerPosition)

                    edtTitulo.setText(title)
                    edtDescripcion.setText(description)
                }
            }
            ...
            if (title.isEmpty()) {
                ...
                if (description.isEmpty()) {
                    ...
                    if (gender.equals("NO SELECCIONADO")) {
                        ...
                        if (image_uri == null) {
                            //post without image
                            uploadData(idPost, gender, title, description, "noImage")
                        } else {
                            uploadData(idPost, gender, title, description, image_uri.toString())
                        }
                    }
                }
            }
        ...
        if (uri != "noImage") {
            ...
            rf.putFile(Uri.parse(uri)).addOnSuccessListener { taskSnapshot ->
                ...
                if (uriTask.isSuccessful) {
                    ...
                    if (idPost != null) {
                        ref.child(idPost).ref.updateChildren(hashMap)
...
//post without image
            ...
            if (idPost != null) {
                ref.child(idPost).ref.updateChildren(hashMap)
                    .addOnSuccessListener {
                        loadingDialog.dismissLoadingAlertDialog()
                        Toast.makeText(this, "Post modificado", Toast.LENGTH_SHORT)
                            .show()
                        edtTitulo.setText("")
                        edtDescripcion.setText("")
                        pImage.setImageURI(null)
